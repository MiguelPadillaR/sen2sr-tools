...
def run_command(command_name: str, args: argparse.Namespace):
    ...
    elif command_name == "download_cubo":
...
def get_parser():
    parser = argparse.ArgumentParser(description="SEN2SR Tools: Command Line Interface for Sentinel-2 Super Resolution.")
    ...
    # =========================================================================
    # 1. get_sr_image (Main Function)
    # =========================================================================
    get_sr_image_parser = subparsers.add_parser(
        "get_sr_image", help="Get SR image from downloaded Sentinel's imagery data and load up SEN2SR model to Super-Resolve it."
    )
    ...
    get_sr_image_parser.add_argument(
        ...
        help="Image bands to retrieve (space-separated, e.g., B08 B02 B03 B04 SCL).", required=False, metavar="STR [...]",
    )
    get_sr_image_parser.add_argument(
    ...
    download_cubo_parser = subparsers.add_parser(
        "download_cubo", help="Download raw Sentinel-2 imagery data using Cubo and filter for cloudless data."
    )
    ...
    download_cubo_parser.add_argument(
        "--bands", nargs='+', default=["B08", "B02", "B03", "B04", "SCL"], help="Image bands to retrieve.", metavar="STR [...]",
    )
    download_cubo_parser.add_argument(
    ...
    download_cubo_parser.add_argument(
        "--polygon-path", type=str, default="", help="GeoJSON to crop SR image filepath. If none is provided, it downloads the image in sizexsize", metavar="STR",
    )
    ...
    crop_parcel_parser = subparsers.add_parser(
        "crop_parcel", help="Crops a parcel from a Super-Resolved TIF using a local GeoJSON file."
    )
